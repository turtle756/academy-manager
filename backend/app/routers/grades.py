...
async def list_grades(
    classroom_id: int | None = None, student_id: int | None = None,
    membership: UserAcademy = Depends(get_membership),
    db: AsyncSession = Depends(get_db),
):
    query = select(Grade).where(Grade.academy_id == membership.academy_id)
    if classroom_id: query = query.where(Grade.classroom_id == classroom_id)
    if student_id: query = query.where(Grade.student_id == student_id)
